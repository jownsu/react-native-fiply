...
const HomeScreen = ({ navigation }) => {
    ...
    const renderMessageList = (item) => (
        <TouchableOpacity
            ...
            onPress={() => navigation.push('MessageScreen', { data: item })}
        >
            <View
                style={[
                    styles.imgContainer,
                    {
                        borderColor: item.isActive ? Colors.primary : Colors.light,
                    },
                ]}
            >
    ...
    return (
        <SafeAreaView statusBarColor={Colors.white}>
            ...
            />
            <Container style={{ paddingHorizontal: 0 }}>{renderList(navIndex)}</Container>
        </SafeAreaView>
    )
}
